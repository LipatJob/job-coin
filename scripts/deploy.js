...
async function main() {
  const contractName = "JobCoin";
  const contractsLocation = "./frontend/src/contracts";
  if (!fs.existsSync(contractsLocation)) {
    fs.mkdirSync(contractsLocation);
  }
  const addressLocation = contractsLocation + `/${contractName}Address.json`;
  const abiLocation = contractsLocation + `/${contractName}Abi.json`;
